...
async function initShader() {
    ...
    gouraudShader = new Shader(gl, gvertexShaderSource, gfragmentShaderSource);
    const pvertexShaderSource = await readShaderFile('shpVert.glsl');
    const pfragmentShaderSource = await readShaderFile('shpFrag.glsl');
    phongShader = new Shader(gl, pvertexShaderSource, pfragmentShaderSource);
}
...
function render() {
    ...
    // Select active shader based on renderingMode
    const activeShader = (renderingMode === 'PHONG') ? phongShader : gouraudShader;
...
async function main() {
    try {
        ...
        gouraudShader.use();
        setLightingUniforms(gouraudShader);

        phongShader.use();
        setLightingUniforms(phongShader);
